...
This project has three separate Twitter/X research providers plus a separate Nansen on-chain provider. Keep them isolated.
...
- Surf X/Twitter posts: use when the user says "через Surf" or chooses `SURF` for Twitter/X posts. Run `surf-search`. Do not call Grok. Do not call SocialData. Do not call direct X/Twitter API.
...
- SocialData X/Twitter search: use when the user says "через SocialData" or chooses `SOCIALDATA`. Run `socialdata-search`. Do not call Grok. Do not call Surf. Do not call direct X/Twitter API.
- Nansen on-chain research: use when the user says "через Nansen", "Nansen", "ончейн", "on-chain", or asks for wallet/token/smart-money data that should come from Nansen. For chat questions, run `nansen-ask` yourself and answer from its output. Use `nansen-agent`, `nansen-token-screener`, or `nansen-call` only for manual endpoint work. Do not call Grok, Surf, SocialData, or direct X/Twitter API.
- Direct Twitter/X API: disabled. If the user explicitly asks for "Twitter API" or direct API, do not call X/Twitter API. Tell them direct Twitter API access is disabled and ask whether they want `GROK`, `SURF`, or `SOCIALDATA` instead.

## Provider Selection Prompt

When provider is missing, ask exactly this and wait for the user's choice:
...
- In Codex chat, do not ask the user to run commands manually. Run `surf-ask` yourself for simple Surf questions, read the output, then answer in a concise human format.
...
- In Codex chat, do not ask the user to run commands manually. Run `nansen-ask` yourself, read the output, then answer in a concise human format.
...
`ask` is a provider selector, not a direct Twitter API command:
