...
- Purpose: локальный CLI-фреймворк для подключения к любому AI-агенту (Codex, Claude, другой agent SDK с доступом к shell), который умеет искать в X/Twitter через выбираемый провайдер (Grok/xAI `x_search`, Surf, SocialData), вести широкий crypto research через AskSurfAI/Surf Data и Chat API, и делать ончейн-ресёрч через Nansen API. Прямой доступ к X/Twitter API v2 отключён по умолчанию.
- Languages: Python 3
...
Этот репозиторий - безопасный шаблон без чужих API-ключей, без личных данных и без истории конкретного инстанса. Каждый пользователь клонирует его, вставляет свои ключи и подключает к своему AI-агенту (Codex, Claude Code, любой agent SDK с доступом к shell/bash-инструменту).
...
- `AGENTS.md` задаёт обязательное правило scope control: агент использует только запрошенные оператором инструменты и ищет только запрошенную информацию; любые сторонние источники, проверки и замены требуют отдельного одобрения.
...
### Codex / любой агент, читающий AGENTS.md

Правила лежат в `AGENTS.md` (общий scope control) и `CODEX.md` (конкретные workflow по каждому провайдеру). Большинство агентов с поддержкой project-level инструкций (Codex CLI, аналогичные agent SDK) подхватывают `AGENTS.md` автоматически из корня репозитория.
...
Use it whenever the user explicitly asks to search through Twitter/X, Grok, Surf, SocialData, or Nansen on-chain data.

For generic Twitter/X search requests, ask the user to choose one provider first: GROK, SURF, or SOCIALDATA.

For Grok requests, run:
python3 -m twitter_research grok-search "USER QUESTION"
(always uses the fixed model grok-4.3, no model flag)
...
Keep modes isolated: never mix providers unless the user explicitly asks to combine sources.
```

`<PROJECT_ROOT>` - путь к папке, куда установлен или склонирован этот репозиторий. Агент должен иметь разрешение на локальный shell/tool use. Если shell недоступен, используйте CLI вручную и передавайте агенту сохранённый JSON из `data/runs/` для анализа.

Подробный гайд - в [docs/agent-integration-guide.md](docs/agent-integration-guide.md).
...
- `docs/agent-integration-guide.md` - гайд для подключения проекта к Codex, Claude или другому локальному агенту
- `AGENTS.md` - правила scope control для агента
...
- `CODEX.md` - рабочие инструкции для агента по Twitter/X research
